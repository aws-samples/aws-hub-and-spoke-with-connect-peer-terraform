...
resource "aws_security_group" "csr_public_interface_security_group" {
  ...
  tags = {
    Name = "CSR Public Interface Security Group"
  }
}
...
resource "aws_security_group" "internal" {
  ...
  vpc_id      = aws_vpc.vpc.id
  ingress {
    description = "Allow all ingress traffic from AWS environment"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
  ...
  egress {
    description = "Allow all egress traffic to AWS environment"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
    cidr_blocks = ["10.0.0.0/8"]
